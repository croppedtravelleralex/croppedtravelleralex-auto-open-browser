...
async fn auto_selection_result_exposes_trust_score_components_and_candidate_preview() {
    ...
    if let Some(factors) = task_json.get("winner_vs_runner_up_diff").and_then(|v| v.get("factors")).and_then(|v| v.as_array()) {
        ...
        assert!(labels.iter().all(|label| matches!(*label, "verify_ok" | "geo_match" | "geo_mismatch" | "region_mismatch" | "upstream_ok" | "raw_score" | "missing_verify" | "stale_verify" | "verify_failed_heavy" | "verify_failed_light" | "verify_failed_base" | "history_risk" | "provider_risk" | "provider_region_risk" | "verify_confidence" | "anonymity" | "probe_latency" | "exit_ip_not_public" | "probe_error_category" | "soft_min_score")));
    }
...
async fn proxy_explain_trust_score_components_roundtrip_as_typed_shape() {
    ...
    for key in [
        "verify_ok_bonus", "verify_geo_match_bonus", "smoke_upstream_ok_bonus", "raw_score_component", "verify_confidence_bonus",
