...
use crate::network_identity::proxy_selection::{apply_proxy_resolution_metadata, proxy_selection_base_where_sql, proxy_selection_order_sql, proxy_long_term_weight_sql, resolved_proxy_json};
use crate::{
    app::state::AppState,
    domain::{
    ...
    let row = match row {
        ...
        None => {
            let query = format!(
                ...
                proxy_selection_order_sql().replace("{long_term_weight}", proxy_long_term_weight_sql())
            );
