...
3. **推进真实 Lightpanda 执行路径硬化**，当前已落下 execution-path hardening v1，并继续完成 runner 可观测性 / artifact 质量 v1：新增 `failure_scope`、`browser_failure_signal`，增强对 runner 失败与浏览器失败的区分能力。
...
- **改进建议：** 下一步最值得做的是 **继续沿 real Lightpanda 主线推进指纹真实消费边界，而不是过早扩浏览器能力面**。
