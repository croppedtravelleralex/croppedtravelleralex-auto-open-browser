...
3. **推进绝对指纹优先开发规则固化**，当前已把 headless Ubuntu 运行约束、磁盘扩容后的取舍更新、性能优化不得退化成伪串行、以及 screenshot/GUI/重视觉能力的降级原则一并写入 fingerprint-first rules，作为后续开发依据。
...
- **改进建议：** 下一步最值得做的是 **严格按新规则落第一批真实实现：指纹字段分层、指纹-代理-地区一致性检查、以及不会退化成伪串行的性能/并发预算**。
