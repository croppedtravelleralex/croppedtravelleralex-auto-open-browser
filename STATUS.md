...
3. **推进绝对指纹优先开发规则固化**，当前已将 fingerprint-first development rules 写入项目规则文档，明确后续功能取舍默认服从“真实指纹消费 / 一致性 / 性能预算优先”，并显式冻结 screenshot / GUI / 重 artifact 这类当前低价值能力。
...
- **改进建议：** 下一步最值得做的是 **按新规则先实现指纹字段优先级分层、指纹-代理-地区一致性检查与性能预算约束，再决定后续动作扩展**。
