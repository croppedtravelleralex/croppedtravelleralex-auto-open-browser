...
3. **推进 provider 级 refresh 范围收窄方案设计**，优先研究 **只先落 provider risk version / dirty 标记 + 懒刷新** 的最小路径，不与 provider_region 一起上。
...
- **改进建议：** 下一步最值得做的是 **把 provider 级 refresh 收窄方案收成最小实现边界：先只落 provider risk version / dirty，不与 provider_region 一起上**。
