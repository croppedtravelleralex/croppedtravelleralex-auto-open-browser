...
REAL_LONGRUN_MODE="${PROXY_VERIFY_REAL_MODE:-${AUTO_OPEN_BROWSER_PROXY_MODE:-demo_public}}"
REAL_LONGRUN_CONFIG="${PROXY_VERIFY_REAL_CONFIG:-${AUTO_OPEN_BROWSER_PROXY_HARVEST_CONFIG:-}}"
...
run_real_longrun() {
  ...
  if [[ -n "${AUTO_OPEN_BROWSER_API_KEY:-}" ]]; then
    cmd+=(--api-key "${AUTO_OPEN_BROWSER_API_KEY}")
  fi
